...
const seed = async ({ topicData, userData, articleData, commentData }: SeedData) => {
  return db
    ...
    .then(() => {
      const formattedArticleData = articleData.map(article => convertTimestampToDate(article));
    ...
    .then(() => {
      const formattedCommentData = commentData.map(comment => convertTimestampToDate(comment));
